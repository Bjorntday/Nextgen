...
const i18n = {
  zh: {
    ...
    chatToolAssets: "素材",
    chatToolLink: "链接",
    chatPlaceholder: "告诉 NextGen 你想做什么...",
    assetsCount: "{count} 个素材",
    msgAssetUploaded: "已上传 {count} 个素材，NextGen 正在解析商品素材。",
    ...
    msgTaskReset: "NextGen 任务已重置。请先上传 1-3 张商品高清图。",
    msgFlowDirect: "已走“有商品图直接生成视频”路径。请上传商品图后直接开始视频生成。",
    msgFlowDraft: "收到你的需求草稿：\"{draft}\"。请先补全 PRD 输入后再生成。",
    msgFlowWelcome: "欢迎使用 NextGen。请上传 1-3 张商品高清图并填写卖点。",
  ...
  en: {
    ...
    chatToolLink: "Link",
    chatPlaceholder: "Tell NextGen what you want to do...",
    assetsCount: "{count} assets",
    msgAssetUploaded: "{count} asset(s) uploaded. NextGen is analyzing product assets.",
    ...
    msgTaskReset: "NextGen task reset. Please upload 1-3 HD product images first.",
    ...
    msgFlowWelcome: "Welcome to NextGen. Please upload 1-3 HD product images and fill in selling points.",
...
function setActiveAsset(assetId) {
  ...
  if (selected.type === "video") {
    ...
    previewHint.textContent =
      currentLang === "zh"
        ? "图片已加载，NextGen AI 可继续生成场景与模特行为。"
        : "Image loaded. NextGen AI can generate scenario + model behavior.";
    currentDuration = 20;
    updateTimecode(0, currentDuration);
  }
}
...
if (from === "direct-upload") {
  taskName.textContent = currentLang === "zh" ? "NextGen 视频任务" : "NextGen Video Task";
  pushMessage("assistant", t("msgFlowDirect"));
  if (draft) {
    chatInput.value = draft.slice(0, 120);
  }
} else if (draft) {
  taskName.textContent = currentLang === "zh" ? "NextGen 商品任务" : "NextGen Product Task";
