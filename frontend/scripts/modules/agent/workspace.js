...
export async function callNextGenWorkflow(action, extra = {}) {
...
export async function hydrateWorkflowTexts(force = false) {
  ...
  try {
    ...
    try {
      const scriptResp = await callNextGenWorkflow("generate_script", {
    ...
    let prompt = "";
    try {
      const promptResp = await callNextGenWorkflow("build_export_prompt", { script_text: script });
