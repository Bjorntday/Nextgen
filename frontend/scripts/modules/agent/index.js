...
async function enhancePromptByAgent() {
  ...
  let templateText = "";
  try {
    const tplResp = await callNextGenWorkflow("build_enhance_template", {
      raw_prompt: raw,
    ...
    try {
      const resp = await fetch(`${window.location.origin}/api/shoplive/image/generate`, {
        ...
        body: JSON.stringify({
          ...
          target_audience: "",
          brand_philosophy: style ? `${style} product storytelling` : "NextGen conversion-oriented product storytelling",
