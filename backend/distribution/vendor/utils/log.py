...
def log_formatter(record: dict) -> str:
    ...
    biz = record["extra"].get("business_name")
    prefix = f"<fg #70acde>[{biz}]</fg #70acde> " if biz else ""
    return f"<fg #70acde>{{time:YYYY-MM-DD HH:mm:ss}}</fg #70acde> | <fg {color}>{{level}}</fg {color}>: {prefix}<light-white>{{message}}</light-white>\n"
...
def create_logger(log_name: str, file_path: str):
    ...
    logger.add(Path(BASE_DIR / file_path), filter=filter_record, level="INFO", rotation="10 MB", retention="10 days", backtrace=True, diagnose=True,
               format="{time:YYYY-MM-DD HH:mm:ss} | {level}: [{extra[business_name]}] {message}\n")
